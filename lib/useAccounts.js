...
function useAccounts({ walletAddress } = {}) {
  ...
  return useQuery({
    ...
    queryFn: async function () {
      ...
      const abi = require(
        `@synthetixio/v3-contracts/${blockchain.chainId}-${blockchain.preset}/AccountProxy.readable.json`
      );
      const address = require(
        `@synthetixio/v3-contracts/${blockchain.chainId}-${blockchain.preset}/meta.json`
